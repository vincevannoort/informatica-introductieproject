...
const { failing, trait } = use('Test/Suite')('Browser')
trait('Test/Browser', {
...
failing('Login fails when using invalid credentials', async ({ browser }) => {
  ...
  await page
    ...
    .assertFn(() => document.location.pathname, '/login')
...
failing('Login successful when using valid credentials', async ({ browser }) => {
  ...
  // check if redirected to dashboard
    .assertFn(() => document.location.pathname, '/')
}).timeout(5000)
...
failing('Add a new relation', async ({ browser }) => {
  ...
  await page
    ...
    .click('button.button.is-link')
    .waitForElement('div.relations-table')
    .pause(10)
