<template>
  <div class="relation-contacts">
    <div v-if="contacts && contacts.length">
      <router-link class="relation-contact" :data-contact="`${contact.first_name} ${contact.last_name}`" tag="div" v-for="contact in contacts" :key="contact.id" :to="`/relations/${$route.params.relation_id}/contacts/${contact.id}`">
        <table>
          <tr>
